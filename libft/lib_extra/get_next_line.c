...
/*   Updated: 2018/11/13 14:04:44 by sgalasso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	buf[GNL_BUFF_SIZE + 1] = {'\0'};
	int			ret[2];
	...
	while (ret[0])
	{
		if (!(*buf))
		{
			if ((ret[0] = read(fd, buf, GNL_BUFF_SIZE)) == 0 && **line)
				return (1);
			else if (ret[0] < 0)
				return (-1);
		}
		...
		if (buf[ret[1]] == '\n')
		{
			ft_strncpy(buf, buf + ret[1] + 1, GNL_BUFF_SIZE);
			return (1);
		}
		ft_strncpy(buf, buf + ret[1], GNL_BUFF_SIZE);
	}
	return (0);
}
